...
const DemandsList = ({ navigation }) => {
  ...
  const loadSearchOption = async () => {
    const [searchOptions] = await Promise.all([
      AsyncStorage.getItem('__demand_options'),
    ]);
    if (searchOptions) {
  ...
  if(openProceed && selectedId){
    ...
    return (
      ...
      >
        <CommonContainer>
          <BOHToolbar>
            <Text selectable={true} style={{fontSize:TextdefaultSize}}>{`Display name:  ${searchOptions.display_name}`}</Text>
            ...
            <Text selectable={true} style={{marginLeft:50, fontSize:TextdefaultSize}}>{`Total Demand:  ${searchOptions.out_amount}`}</Text>
          </BOHToolbar>
          {tableElement}
        </CommonContainer>
      </BasicLayout>
