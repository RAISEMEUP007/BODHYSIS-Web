...
const Exclusions = ({ DiscountCodeId, updateExclusionTrigger, setUpdateExclusionTrigger, openExclusionModal, editExclusion }) => {
  ...
  const renderTableData = () => {
    ...
    if(tableData.length > 0){
      tableData.map((item, index) => {
        rows.push( 
          <View key={index} style={styles.tableRow}>
            ...
            <View style={[styles.cell]}>
              <Text>{new Date(item.from_date).toLocaleString('en-US', {year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', hour12: true })}</Text>
            </View>
            <View style={[styles.cell]}>
              <Text>{new Date(item.to_date).toLocaleString('en-US', {year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', hour12: true })}</Text>
            </View>
            <View style={[styles.IconCell]}>
              <TouchableOpacity onPress={()=>{editExclusion(item)}}>
                <FontAwesome5 size={TextMediumSize} name="edit" color="black" />
              </TouchableOpacity>
            </View>
            <View style={[styles.IconCell]}>
              <TouchableOpacity onPress={()=>{removeExclusion(item.id)}}>
      ...
      <View style={{alignItems:'center', paddingTop:10}}><Text>No Address</Text></View>
    }
    return <>{rows}</>;
  };
